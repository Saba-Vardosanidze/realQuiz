...
import {quizMedium} from "../../api/quizMedium";
import {useState} from "react";
const QuizMediumQuestion = () => {
  ...
  return (
    <div className="flex flex-col justify-between items-center gap-[80px] m-auto mt-[40px] lg:mt-[140px] w-full max-w-[880px]">
      <div className="flex flex-wrap justify-center gap-[50px] w-full max-w-[1400px]">
        {data.map((res, index) => (
          <div key={index}>
            <div className="flex justify-center items-center hover:shadow border-[1px] hover:border-green-700 border-blue-950 rounded-[15px] w-[410px] h-[410px] overflow-hidden">
              <img
                onClick={() => scoreGenerator(res.isDog)}
                className="blur-md w-full lg:w-[400px] h-full lg:h-[400px] cursor-pointer"
                src={res.url}
                alt="catOrDogImg"
              />
            </div>
...
export default QuizMediumQuestion;
